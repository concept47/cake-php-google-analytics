 ...
 */
	function login($user = null, $password = null) {
		...
		$post = array(
			'continue' => 'https://www.google.com/analytics/settings/?et=reset&hl=en-US',
			'service' => 'analytics',
			'nui' => 'hidden',
			'hl' => 'en-US',
			'GA3T' => 'ouVrvynQwUs',
 ...
 */
	function listSources($refresh = false) {
		...
		foreach ($accounts as $i => $account) {
			...
			if ($i != 0) {
				$response = $this->Http->get('https://www.google.com/analytics/settings/?scid='.$id.'&ns=100');
			}
 ...
 */
	function addProfile($profile) {
		...
		$response = $this->Http->post('https://www.google.com/analytics/settings/?scid='.$profile['account'], $post);
